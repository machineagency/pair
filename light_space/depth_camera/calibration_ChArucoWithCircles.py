...
objp = np.zeros((CHARUCOBOARD_COLCOUNT*CHARUCOBOARD_ROWCOUNT,3), np.float32)
objp[:,:2] = np.mgrid[0:CHARUCOBOARD_COLCOUNT,0:CHARUCOBOARD_ROWCOUNT].T.reshape(-1,2)
...
if not os.path.exists('./calibration/MarkerCalibration.pckl'):
    ...
    cap = cv2.VideoCapture('trial_4.mov')
    validCaptures = 0
    ...
    # Hack: set fy to be some proportion of fx because I think fy is wrong
    cameraMatrix[1,1] = cameraMatrix[0, 0] * (1080 / 720)
    ...
    print('Camera calibration found, continuing to projector calibration.')
...
while cap.isOpened():
    ...
    max_num_markers = 17
    reasonable_num_markers = 12
    if ids is not None and len(ids) >= max_num_markers:
        # Get charuco corners and ids from detected aruco markers
        response, charuco_corners, charuco_ids = aruco.interpolateCornersCharuco(
                markerCorners=corners,
                markerIds=ids,
        ...
        if pose:
            ...
            img = aruco.drawAxis(img, cameraMatrix, distCoeffs, rvec, tvec, 0.022)
...
K_proj[1][2] = 1
K_proj[2][2] = 1
dist_coef_proj = np.ones(shape=(1,4))

print(objpoints[0])
print(imgpointsProj[0])
...
ret, K, dist_coef, K_proj, dist_coef_proj, proj_R, proj_T, E, F = cv2.stereoCalibrate(
    objpoints,
    imgpoints,
    imgpointsProj,
    cameraMatrix,
