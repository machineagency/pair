...
export const mm = (valueInMm) => {
    return valueInMm * MM_TO_PX;
};
export const px = (valueInPx) => {
...
export class Tabletop {
    constructor(machine) {
        ...
        this.toolpaths = [];
        this.workEnvelope = new WorkEnvelope(this, machine.workEnvelopeDimensions.x, machine.workEnvelopeDimensions.y);
    ...
    addVizWithName(visualization, name) {
        visualization.name = name;
        this.vizLayer.addChild(visualization);
    }
    removeVizWithName(name) {
...
export class Toolpath {
    constructor(geometryUrl, instructions) {
...
export class Region {
    ...
    drawOnTabletop(tabletop) {
        let from = this.corners[0].paperPoint;
        let to = this.corners[3].paperPoint;
...
export class Camera {
    ...
    async findFaceRegions() {
        ...
        if (response.ok) {
            ...
            regions = resJson.results.map((obj, idx) => {
                let tl = new Point(Math.round(obj.topLeftX * this.imageToTabletopScale.x), Math.round(obj.topLeftY * this.imageToTabletopScale.y));
                let tr = new Point(Math.round((obj.topLeftX + obj.width) * this.imageToTabletopScale.x), Math.round(obj.topLeftY * this.imageToTabletopScale.y));
                let bl = new Point(Math.round(obj.topLeftX * this.imageToTabletopScale.x), Math.round((obj.topLeftY + obj.height) * this.imageToTabletopScale.y));
                let br = new Point(Math.round((obj.topLeftX + obj.width) * this.imageToTabletopScale.x), Math.round((obj.topLeftY + obj.height) * this.imageToTabletopScale.y));
...
export class Geometry {
    ...
    placeAt(placementPoint, tabletop) {
        if (!this.paperGroup) {
        ...
        return new Promise((resolve) => {
            this.tabletop.project.importSVG(filepath, {
                ...
                onLoad: (item, svgString) => {
                    ...
                    item.position = new paper.Point(item.bounds.width * 0.5 + this.tabletop.workEnvelope.anchor.x, item.bounds.height * 0.5 + this.tabletop.workEnvelope.anchor.y);
...
export class Machine {
    ...
    async compileGeometryToToolpath(geometry) {
        // Credit: https://github.com/yoksel/url-encoder/ .
        const urlEncodeSvg = (data) => {
            const symbols = /[\r\n%#()<>?[\\\]^`{|}]/g;
            data = data.replace(/"/g, `'`);
    ...
    /* DEPRECATED */
    async _fetchPreviewUrl(toolpath) {
        // Credit: https://github.com/yoksel/url-encoder/ .
        const urlEncodeSvg = (data) => {
            const symbols = /[\r\n%#()<>?[\\\]^`{|}]/g;
            data = data.replace(/"/g, `'`);
    ...
    plotToolpathOnTabletop(toolpath, tabletop) {
